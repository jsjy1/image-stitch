...
def correlation1(feature1, feature2, seach_range):
    """calculate correlation matrix between feature1 and feature2(both are l2 normalized)"""
    b,c,h,w = feature1.shape
    ...
    for i in range(0,h-seach_range+1,seach_range):
        for j in range(0,w-seach_range+1,seach_range):
            for z in range(b):
                temp = nn.functional.conv2d(feature2[z].unsqueeze(0),feature1[z,:,i:i+seach_range,j:j+seach_range].unsqueeze(0),stride=seach_range).reshape(-1)
    ...
    return  torch.cat((temp1,temp),dim=1)  #由于每个点里包含seach_range**2个求和
...
class Net(nn.Module):
    ...
    #计算相关性，回归偏移量
        ...
        search_range = 2  #16
        cor1 = correlation1(l2_normalize(feature1[-1]), l2_normalize(feature2[-1]), search_range)
        #回归偏移量，计算单应矩阵，warp上一级特征图
        offset1 = self.regression1(cor1)  *8
        patch_size = 32
        ...
        search_range = 4 #8
        cor2 = correlation1(l2_normalize(feature1[-2]), feature22_warp, search_range)
        # 回归偏移量，计算单应矩阵，warp上一级特征图
        offset2 = self.regression2(cor2) *4
        patch_size = 64
        ...
        search_range = 4 #4
        cor3 = correlation1(l2_normalize(feature1[-3]), feature23_warp, search_range)
        # 回归偏移量，计算单应矩阵，warp上一级特征图
    ...
    #计算最后的H
        ...
        # print('cor1: ',cor1.max().detach().cpu().numpy(),' cor2: ',cor2.max().detach().cpu().numpy(),' cor3: ',cor3.max().detach().cpu().numpy())
        # print('offset1:',offset1.max().detach().cpu().numpy(),' offset2:',offset2.max().detach().cpu().numpy(),' offset3:',offset3.max().detach().cpu().numpy())
...
class Setup():
    ...
    istrain = True
    show_img = False                # 是否每轮展示第一张图片
    batch_size = 5
    pth_file = './5.pth'
    save_pth_file = './5.pth'
    warp_loss = True   # 是否使用warp损失
