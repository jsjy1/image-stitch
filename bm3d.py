"""
https://blog.csdn.net/weixin_44580210/article/details/107139590
"""
# -*- coding: utf-8 -*-
"""
*BM3D算法简单实现,主要程序部分
...
def init(img, _blk_size, _Beta_Kaiser):
    ...
    m_weight = numpy.matrix(numpy.zeros(m_shape, dtype=float))
    ...
    return m_img, m_weight, m_Kaiser
...
def Aggregation_hardthreshold(_similar_blocks, blk_positions, m_basic_img, m_weight_img, _nonzero_num, Count, Kaiser):
    '''
    ...
    *对这些块，用非零项的权重乘以凯撒窗之后再分别放回原位
    ...
    if _nonzero_num < 1:
        _nonzero_num = 1
    block_weight = (1. / (sigma ** 2 * _nonzero_num)) * Kaiser
    for i in range(Count):
        point = blk_positions[i, :]
        tem_img = block_weight * cv2.idct(_similar_blocks[i, :, :])
        m_basic_img[point[0]:point[0] + _shape[1], point[1]:point[1] + _shape[2]] += tem_img
        m_weight_img[point[0]:point[0] + _shape[1], point[1]:point[1] + _shape[2]] += block_weight
...
def BM3D_1st_step(_noisyImg):
    ...
    # 初始化几个数组
    # 空图像、空权重表、凯撒窗
    Basic_img, m_weight, m_Kaiser = init(_noisyImg, Step1_Blk_Size, Beta_Kaiser)
    ...
    for i in range(int(Width_num + 2)):
        for j in range(int(Height_num + 2)):
            ...
            Aggregation_hardthreshold(Similar_Blks, Positions, Basic_img, m_weight, statis_nonzero, Count, m_Kaiser)
    Basic_img[:, :] /= m_weight[:, :]
...
def Step2_3DFiltering(_Similar_Bscs, _Similar_Imgs):
    ...
    *返回的Wiener_weight用于后面Aggregation
    '''
    m_Shape = _Similar_Bscs.shape
    Wiener_weight = numpy.zeros((m_Shape[1], m_Shape[2]), dtype=float)
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            ...
            Wiener_weight[i, j] = m_weight

            #if m_weight != 0: Wiener_weight[i, j] = 1. / (m_weight ** 2 * sigma ** 2)
            # else:
            #     Wiener_weight[i, j] = 10000
    ...
    return _Similar_Bscs, Wiener_weight


def Aggregation_Wiener(_Similar_Blks, _Wiener_weight, blk_positions, m_basic_img, m_weight_img, Count, Kaiser):
    '''
    *对3D变换及滤波后输出的stack进行加权累加,得到初步滤波的图片
    *_similar_blocks:相似的一组block,这里是频域的表示
    *对于最后的块，乘以凯撒窗之后再输出
    ...
    block_weight = _Wiener_weight * Kaiser

    for i in range(Count):
        point = blk_positions[i, :]
        tem_img = _Wiener_weight * cv2.idct(_Similar_Blks[i, :, :]) * Kaiser
        m_basic_img[point[0]:point[0] + _shape[1], point[1]:point[1] + _shape[2]] += tem_img
        m_weight_img[point[0]:point[0] + _shape[1], point[1]:point[1] + _shape[2]] += block_weight
...
def BM3D_2nd_step(_basicImg, _noisyImg):
    ...
    # 初始化几个数组
    m_img, m_weight, m_Kaiser = init(_noisyImg, block_Size, Beta_Kaiser)
    ...
    for i in range(int(Width_num + 2)):
        for j in range(int(Height_num + 2)):
            ...
            Similar_Blks, Wiener_weight = Step2_3DFiltering(Similar_Blks, Similar_Imgs)
            Aggregation_Wiener(Similar_Blks, Wiener_weight, Positions, m_img, m_weight, Count, m_Kaiser)
    m_img[:, :] /= m_weight[:, :]
...
if __name__ == '__main__':
    ...
    # cv2.imshow("Basic3.jpg", Basic_img)
    cv2.imshow("Basic3.jpg", Basic_img.astype(numpy.uint8)) # 123
    cv2.waitKey(1)
    ...
    # cv2.imshow("Final3.jpg", Final_img)
    cv2.imshow("Final3.jpg", Final_img.astype(numpy.uint8))  # 123
