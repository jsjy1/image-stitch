...
def correlation(feature1, feature2, seach_range):
    ...
    return nn.LeakyReLU()(torch.cat(cor_matrix,dim=1))   #沿着通道方向拼接  并取leakyrelu
...
class Net(nn.Module):
    ...
    #计算相关性，回归偏移量
        ...
        patch_size = 32
        src_p = torch.Tensor([[0,0,0,patch_size-1,patch_size-1,0,patch_size-1,patch_size-1]])  #上一级特征图尺寸 左上 右上  左下  右下
        ...
        patch_size = 64
        src_p = torch.Tensor([[0, 0, 0, patch_size - 1, patch_size - 1, 0, patch_size - 1,
                               patch_size - 1]])  # 上一级特征图尺寸 左上 右上  左下  右下
        # src_p = torch.cat((src_p, src_p, src_p, src_p), dim=0)
        src_p = src_p.expand(b, src_p.shape[-1])
...
class Myloss(nn.Module):
    ...
    def forward(self,img_pair,H1,H2,H3):
        ...
        loss_warp = b*(16 * patch_size ** 2 / (torch.sum(E1) + decimal) + 16 * patch_size ** 2 / (torch.sum(E2) + decimal) + 16 * patch_size ** 2 / (torch.sum(E3) + decimal))

        E1 = E1 * img1
        E2 = E2 * img1
        E3 = E3 * img1

        loss_content = 16*torch.sum(torch.abs(img2_1-E1)) + 4*torch.sum(torch.abs(img2_2-E2)) + torch.sum(torch.abs(img2_3-E3))    #让数字大一点  不用mean了

        print(loss_content.cpu().detach().numpy(),"    ",loss_warp.cpu().detach().numpy(),end='  ')
        return  loss_content #+ loss_warp
...
def show(img_pair,H1,H2,H3):
    ...
    E = torch.ones(img1.unsqueeze(0).shape, dtype=torch.float32)
...
class Setup():
    ...
    show_img = True
    batch_size = 1
    pth_file = './225_train.pth'


if __name__ == '__main__':

    setup = Setup()
    ds = Dataloader('./dataset',is_train=False)
    train_loader = DataLoader(dataset=ds,batch_size=1,shuffle=True,num_workers=0,drop_last=False)
    # vis = visdom.Visdom(env=u'test1')
    # for img_pair  in train_loader:
    #     img_pair = (img_pair.squeeze()+1)*127.5
    #     img_pair = img_pair.unsqueeze(1)
    #     vis.images(img_pair,nrow=1,win='img',opts={'title':'img'})
    #     cv2.waitKey(1000)
    ...
    # net = Net().to(device)
    # net.load_state_dict(torch.load('./2.pth').cpu())
    ...
    #     loss_temp = 0
    #     for img_pair in tqdm(train_loader):
    ...
    #     print('epoch:%d    loss:%.5f   time:%.5f'%(i,loss,time.time()-start))
    #     start = time.time()
    #
    # a = 1